...
fn process_simd_chunks(
    ...
    for chunk_idx in 0..chunk_count {
        ...
        while delimiters != 0 {
            ...
            if is_newline {
                if s.fields_in_row == 0 && s.field_start == abs_pos && !s.in_quoted {
                    s.field_start = abs_pos + 1;
                    if s.field_start < len && abs_pos < len && bytes[abs_pos] == b'\r'
                        && bytes[s.field_start] == b'\n'
                    {
                        s.field_start += 1;
                        let nl_bit = s.field_start - 1 - base;
                        if nl_bit < 64 { delimiters &= !(1u64 << nl_bit); }
                    }
                    s.after_comma = false;
                    if s.field_start < len && bytes[s.field_start] == b'"' {
                        s.in_quoted = true;
                ...
                emit_field(bytes, buf, buf_len, &mut s, field_end, quoted, has_esc, true);
                if s.full { break; }
                s.is_header = false;
                s.fields_in_row = 0;
                s.after_comma = false;
                s.field_start = abs_pos + 1;
                if s.field_start < len && bytes[abs_pos] == b'\r'
                    && bytes[s.field_start] == b'\n'
                {
                    s.field_start += 1;
                    let nl_bit = s.field_start - 1 - base;
                ...
                emit_field(bytes, buf, buf_len, &mut s, field_end, quoted, has_esc, false);
                if s.full { break; }
                s.fields_in_row += 1;
                s.after_comma = true;
                s.field_start = abs_pos + 1;
    ...
    if !s.full {
        ...
        finish_scalar(bytes, buf, &mut s, pos);
    }

    write_done(buf, &mut s);
    ...
    process_simd_chunks(
        ...
        |chunk_idx| {
            ...
            for sub in 0..4u32 {
                ...
                let shift = sub * 16;
                cm |= (_mm_movemask_epi8(_mm_cmpeq_epi8(_mm_and_si128(classified, comma_bit), comma_bit)) as u16 as u64) << shift;
                qm |= (_mm_movemask_epi8(_mm_cmpeq_epi8(_mm_and_si128(classified, quote_bit), quote_bit)) as u16 as u64) << shift;
                nm |= (_mm_movemask_epi8(_mm_cmpeq_epi8(_mm_and_si128(classified, newline_bit), newline_bit)) as u16 as u64) << shift;
            }
    ...
    process_simd_chunks(
        ...
        |chunk_idx| {
            ...
            for sub in 0..4u32 {
                ...
                let shift = sub * 16;
                cm |= (u8x16_bitmask(u8x16_eq(v128_and(classified, comma_val), comma_val)) as u64) << shift;
                qm |= (u8x16_bitmask(u8x16_eq(v128_and(classified, quote_val), quote_val)) as u64) << shift;
                nm |= (u8x16_bitmask(u8x16_eq(v128_and(classified, newline_val), newline_val)) as u64) << shift;
            }
...
fn finish_scalar(bytes: &[u8], buf: &mut [u8], s: &mut ParseState, mut pos: usize) {
    let len = bytes.len();
    let buf_len = buf.len().saturating_sub(9);
    ...
    while pos < len {
        ...
        if s.in_quoted {
            ...
            if b == b'"' {
                ...
                if is_end {
                    ...
                    emit_field(bytes, buf, buf_len, s, field_end, true, s.has_escapes, is_newline);
                    if s.full { return; }
                    if is_newline {
                        s.is_header = false;
                        s.fields_in_row = 0;
                    ...
                    else { skip_newline_at(bytes, &mut pos); skip_blank_lines(bytes, &mut pos); }
                    if is_newline {
                        s.field_start = pos;
                        s.finish_row();
                    }
        ...
        match memchr::memchr3(b',', b'\n', b'\r', &bytes[pos..]) {
            Some(off) => {
                ...
                if delim == b',' {
                    emit_field(bytes, buf, buf_len, s, delim_pos, false, false, false);
                    if s.full { return; }
                    s.fields_in_row += 1;
                    s.after_comma = true;
                    pos = delim_pos + 1;
                    ...
                    emit_field(bytes, buf, buf_len, s, delim_pos, false, false, true);
                    if s.full { return; }
                    s.is_header = false;
                    s.fields_in_row = 0;
                    s.after_comma = false;
                    pos = delim_pos;
                    skip_newline_at(bytes, &mut pos);
    ...
    if !s.full {
        if s.field_start < len {
            emit_field(bytes, buf, buf_len, s, pos, s.in_quoted, s.has_escapes, true);
        } else if s.after_comma {
            emit_field(bytes, buf, buf_len, s, s.field_start, false, false, true);
        }
    }
}

fn skip_newline_at(bytes: &[u8], pos: &mut usize) {
    let len = bytes.len();
    if *pos < len && bytes[*pos] == b'\r' { *pos += 1; }
    if *pos < len && bytes[*pos] == b'\n' { *pos += 1; }
}

fn skip_blank_lines(bytes: &[u8], pos: &mut usize) {
    let len = bytes.len();
    while *pos < len && (bytes[*pos] == b'\n' || bytes[*pos] == b'\r') {
        skip_newline_at(bytes, pos);
    }
}
...
#[inline(always)]
fn emit_field(
    bytes: &[u8], buf: &mut [u8], buf_len: usize, s: &mut ParseState,
    end: usize, quoted: bool, has_escapes: bool, is_last: bool,
) {
    let start = s.field_start;
    let flen = end.saturating_sub(start);

    // check if buffer can fit this field (9 bytes, or more for escaped strings)
    if s.wp + 9 > buf_len {
        s.rollback_row();
        return;
    }
    ...
    let slice = if flen > 0 && start + flen <= bytes.len() { &bytes[start..start + flen] } else { &[] as &[u8] };
    ...
    if !quoted && !has_escapes {
        ...
        if flen > 0 && (slice[0].is_ascii_digit() || slice[0] == b'-' || slice[0] == b'+' || slice[0] == b'.') {
...
fn is_pure_integer(slice: &[u8]) -> bool {
    let start = if !slice.is_empty() && (slice[0] == b'-' || slice[0] == b'+') { 1 } else { 0 };
    start < slice.len() && slice[start..].iter().all(|b| b.is_ascii_digit())
}

#[inline(always)]
fn write_slice(buf: &mut [u8], s: &mut ParseState, op: u8, offset: usize, length: usize, is_last: bool) {
...
fn write_str_escaped(bytes: &[u8], buf: &mut [u8], buf_len: usize, s: &mut ParseState, abs_start: usize, rel_start: usize, flen: usize, is_last: bool) {
    ...
    while let Some(qi) = memchr::memchr(b'"', &slice[search_from..]) {
        ...
        if i + 1 < slice.len() && slice[i + 1] == b'"' {
            ...
            if s.wp + 9 > buf_len { s.rollback_row(); return; }
...
#[inline(always)]
fn write_done(buf: &mut [u8], s: &mut ParseState) {
    buf[s.wp] = OP_EOF;
